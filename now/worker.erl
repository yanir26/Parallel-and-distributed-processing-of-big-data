...
read_file_and_send_the_data(MY_ID,List_Of_Workers,Node_Of_Master)->
  ...
  [ my_cast(Worker,broadcast_finish_to_read_file) || Worker <- List_Of_Workers],
  %gen_server:cast({Node_Of_Master,Node_Of_Master},broadcast_finish_to_read_file),
  my_cast(Node_Of_Master,broadcast_finish_to_read_file),
...
work_on_line(Input,List_Of_Workers)->
  ...
  %[gen_server:cast({for_which_worker(X,List_Of_Workers),for_which_worker(X,List_Of_Workers)},{remoteUpdate,X,Line -- [X]})|| X <- Line,X =/= []]. % all_permutations
   [my_cast(for_which_worker(X,List_Of_Workers),{remoteUpdate,X,Line -- [X]})|| X <- Line,X =/= []]. % all_permutations
      ...
      case maps:get(Input,History,notfound) of 
        ...
        _->
          %gen_server:cast({Source_Node,Source_Node},{mission_accomplished,Source_Pid,{ask_already,Input}}),
           my_cast(Source_Node,{mission_accomplished,Source_Pid,{ask_already,Input}}),
          manage_requests_fun(Structure,List_Of_Workers,List_Of_Processes,History,Responsibilities)
  ...
  if
    ((Partners =:= notfound) or (Partners =:= [])) ->
      ...
      %gen_server:cast({Source_Node,Source_Node},{mission_accomplished,Source_Pid,{notfound,Input}});
       my_cast(Source_Node,{mission_accomplished,Source_Pid,{notfound,Input}});

    Depth =:= ?DEPTH + 1 ->
      %gen_server:cast({Source_Node,Source_Node},{mission_accomplished,Source_Pid,{[],Input}});
       my_cast(Source_Node,{mission_accomplished,Source_Pid,{[],Input}});
    true ->
      ...
      List_Of_Relevant_Partners = [my_cast(for_which_worker(Person,List_Of_Workers),{incoming_input,node(),self(),Person,Depth + 1,Fathers ++ [Person]  ++ Partners}) || Person <- Partners,not(lists:member(Person,Fathers))],
      ...
      if
        ...
        true ->
         ...
         my_cast(Source_Node,{mission_accomplished,Source_Pid,{Res,Input}})
      end
  end,
  finito.
...
my_cast(Destenation,Message)->
  try gen_server:cast({Destenation,Destenation},Message) of
    _->ok
  catch
    error:_Error -> timer:sleep(3000);
    exit:_Exit	->   timer:sleep(3000);
    throw:_Throw->  timer:sleep(3000)
  end.
...
handle_call(construction_from_master, _From, [Node_Of_Master]) ->
  ...
  Result =   try gen_server:call({Node_Of_Master,Node_Of_Master},{ask_to_construction_from_master,node()},infinity) of
	    X->
             State = lists:nth(2,X) ++ [Node_Of_Master],
             {reply, X, State}
	  catch
	    error:_Error ->  {noreply,  [Node_Of_Master]};
	    exit:_Exit	->    {noreply,  [Node_Of_Master]};
	    throw:_Throw->   {noreply,  [Node_Of_Master]}
	  end,
